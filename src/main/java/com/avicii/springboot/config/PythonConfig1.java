...
public class PythonConfig1 {
    ...
    public static String getPyResult(String protocal, String dIp, Integer max_ttl, Float timeout) throws InterruptedException, IOException {
        String outputString = null;
        int exitCode = 0;
        try {
            // 构建 Python 命令及参数
            String pythonScript = "D:\\Projects\\SystemA\\springboot\\springboot\\src\\main\\python\\main.py";
            ProcessBuilder processBuilder =new ProcessBuilder(pyPath, pythonScript, protocal, dIp, String.valueOf(max_ttl), String.valueOf(timeout));
            // 启动进程并执行命令
            Process process =processBuilder.start();
            // 获取命令输出
            BufferedReader reader =new BufferedReader(new InputStreamReader(process.getInputStream()));
    ...
    public static String KeyNodeIdentification(String jsonPath) throws InterruptedException, IOException {
        String outputString = null;
        int exitCode = 0;
        try {
            // 构建 Python 命令及参数
            String pythonScript = "D:\\Projects\\SystemA\\springboot\\springboot\\src\\main\\python\\test.py";
            ProcessBuilder processBuilder =new ProcessBuilder(pyPath, pythonScript, jsonPath);
            // 启动进程并执行命令
            Process process =processBuilder.start();
            // 获取命令输出
            BufferedReader reader =new BufferedReader(new InputStreamReader(process.getInputStream()));
